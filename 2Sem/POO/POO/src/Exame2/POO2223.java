...
public class POO2223 {
	...
	public static void main(String[] args) throws FileNotFoundException {
		PrintStream fl = new PrintStream(new File("src/Exame/POO_2223EP.txt"));
	...
	private static void alinea2(PrintStream out) {
		...
		*/
		...
		while (sc.hasNextLine()) {
			...
			UnitType tipo = UnitType.parse(parts[3]);
			String[] dims = parts[4].split("x");
			int[] dim = new int[3];
			for (int i = 0; i < 3; i++) {
				dim[i] = Integer.parseInt(dims[i]);
			}
			Client client = storageService.getClient(taxID);
			storageService.rentStorageUnit(client, unit, startDate, Duration);
		}
